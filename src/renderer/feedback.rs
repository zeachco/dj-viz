...
impl FeedbackRenderer {
    ...
    /// Render a frame with feedback effect and output to the provided frame view.
    ///
    /// # Arguments
    /// * `device` - wgpu device
    /// * `queue` - wgpu queue
    /// * `draw` - nannou Draw with visualization commands
    ...
    pub fn render(
        ...
        // Pass 2: Draw current visualization on top (with alpha blending)
        self.draw_renderer.render_to_texture(device, &mut encoder, draw, &self.textures[curr_idx]);

        // Pass 3: Copy feedback result to frame
        let reshaper = wgpu::TextureReshaper::new(
            device,
            &self.texture_views[curr_idx],
            1,
            wgpu::TextureSampleType::Float { filterable: true },
            frame_sample_count,
            frame_format,
